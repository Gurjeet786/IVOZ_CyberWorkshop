...
| Tool | Used In | Install Source |
|---|---|---|
| Arduino IDE (2.x) | All coding practicals | arduino.cc/en/software |
| ESP32 Board Package (via Boards Manager) | All | `https://raw.githubusercontent.com/espressif/arduino-esp32/gh-pages/package_esp32_index.json` |
| Libraries: `DHT sensor library`, `MFRC522`, `PubSubClient`, `ESP32 BLE Arduino` (bundled) | P2, P9, P7/P8, P10 | Arduino Library Manager |
| Wireshark | P4, P8 | wireshark.org |
| Python 3.x + `pip install esptool paho-mqtt requests` | P5, P11, P13 | python.org |
| Mosquitto broker (+ `mosquitto_pub`/`mosquitto_sub` CLI) | P7, P8 | mosquitto.org |
| OpenSSL (usually pre-installed on Linux/Git-Bash on Windows) | P8 | — |
| ESP32Marauder firmware + flasher | P5, P6 | github.com/justcallmekoko/ESP32Marauder |
| nRF Connect for Mobile (Android/iOS, students' own phones) | P10 | Play Store / App Store |
| Git / GitHub Desktop (optional, to distribute this manual + code to students) | All | — |
...
3. Unauthorized access to computer systems, networks, and wireless communications is a criminal offence under the **Information Technology Act, 2000 (India)** — Sections 43 and 66 — and under equivalent computer-misuse laws elsewhere. Everything taught today is intended to build **defensive understanding** ("attacker mindset for defense"), not to be replicated outside this lab.
...
**Step-by-Step:**
1. In Arduino IDE: `Tools > Manage Libraries` → search **DHT sensor library** by Adafruit → Install (accept the "Adafruit Unified Sensor" dependency prompt).
...
**Step-by-Step:**
1. Install library: `Tools > Manage Libraries` → search **MFRC522** by GithubCommunity → Install.
2. Wire the module as per the table above.
...
Close the day by mapping every practical back to the industry-standard **OWASP IoT Top 10** vulnerability classes — this gives students a professional vocabulary for their reports/resumes:
...
- **ESP32Marauder** (WiFi/BT recon & testing firmware): github.com/justcallmekoko/ESP32Marauder
- **MFRC522 Arduino Library** (RFID): github.com/miguelbalboa/rfid
- **OWASP IoT Top 10**: owasp.org/www-project-internet-of-things
- **ESP-IDF Secure Boot & Flash Encryption docs**: docs.espressif.com (search "Secure Boot V2", "Flash Encryption")
- **nRF Connect for Mobile**: Nordic Semiconductor, available on Play Store / App Store
- Recommend students push their own solved-CTF write-ups (P14) to a personal GitHub as a portfolio piece for placements/interviews.
